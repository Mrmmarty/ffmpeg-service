    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      if (segment.textOverlay) {
        ...
        for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
          ...
          const yFinal = lineY
          const rawYExpr = `if(lt(t,${slideStart.toFixed(3)}),${yStart.toFixed(3)},if(lt(t,${slideEnd.toFixed(3)}),${yStart.toFixed(3)}+(${yFinal.toFixed(3)}-${yStart.toFixed(3)})*((t-${slideStart.toFixed(3)})/${fadeInDuration.toFixed(3)}),${yFinal.toFixed(3)}))`
          ...
          const rawAlphaExpr = `if(lt(t,${slideStart.toFixed(3)}),0,if(lt(t,${slideEnd.toFixed(3)}),(t-${slideStart.toFixed(3)})/${fadeInDuration.toFixed(3)},if(lt(t,${fadeOutStart.toFixed(3)}),1,if(lt(t,${fadeOutEnd.toFixed(3)}),1-((t-${fadeOutStart.toFixed(3)})/${fadeOutDuration.toFixed(3)}),0))))`
          
          // Wrap expressions in single quotes and escape commas to prevent filter parser errors
          const yExpr = `'${rawYExpr.replace(/,/g, '\\,')}'`
          const alphaExpr = `'${rawAlphaExpr.replace(/,/g, '\\,')}'`
